...
public class APITest {
    ...
    //private final UDFS udfs = new UDFS("http://########",5001,false);
    private final UDFS udfs = new UDFS("https://########",5001,true);
    ...
    public void pushSingleFileTest() throws IOException {
        NamedStreamable.ByteArrayWrapper file = new NamedStreamable.ByteArrayWrapper("udfs.txt", "welcome udfs! udfs rocks!".getBytes());
        List<MerkleNode> addParts = udfs.add(file, true);//push方法已经remove代理节点的文件，所以不用删除代理节点
        MerkleNode filePart = addParts.get(0);
        MerkleNode dirPart = addParts.get(1);
    ...
    public void pushFileTest() throws IOException {
        ...
        //要添加文件使用
        //NamedStreamable.FileWrapper file = new NamedStreamable.FileWrapper(new File("F:/区块链视频资料/eos开发视频/03 - 账户体系与安全设计/视频/01_账户体系与安全设计.mp4"));
        NamedStreamable.FileWrapper file = new NamedStreamable.FileWrapper(new File("C:/Users/Allen/Desktop/1.jpg"));
        //NamedStreamable.FileWrapper file = new NamedStreamable.FileWrapper(new File("F:/区块链视频资料/go/第5天视频/05_数组越界导致panic.avi"));
        //添加文件到IPFS返回HASH值
        List<MerkleNode> addParts = udfs.push(file);
        //List<MerkleNode> addParts = udfs.add(file);
        long endTime = fromDateStringToLong(new SimpleDateFormat("yyyy-MM-dd hh:mm:ss:SSS").format(new Date()));
        //输出HASH值
        System.out.println("hash:"+addParts.get(0).hash+",backup:"+addParts.get(0).backup+"上传所用的时间:"+(endTime - startTime));
    }
...
//            byte[] data = "some random textual data".getBytes();
    ...
    public void objectTest() throws IOException {
        ...
        Multihash pointer = Multihash.fromBase58("QmeAsSG3dCnotJwmhhAGnydjEJTgQdFg6d6eg5822uc9AB");
    ...
    @Test
    public void objectCat() throws IOException {
        long startTime =fromDateStringToLong(new SimpleDateFormat("yyyy-MM-dd hh:mm:ss:SSS").format(new Date()));
        MerkleNode _new = udfs.object._new(Optional.empty());
        Multihash pointer = Multihash.fromBase58("QmeAsSG3dCnotJwmhhAGnydjEJTgQdFg6d6eg5822uc9AB");
        byte[] fileContents = udfs.cat(pointer);
        String result=new String(fileContents,"UTF-8");
        long endTime = fromDateStringToLong(new SimpleDateFormat("yyyy-MM-dd hh:mm:ss:SSS").format(new Date()));
        System.out.println("获取的内容："+result+"获取内容所用的时间:"+(endTime-startTime));
        //List<MerkleNode> newPointer2 = udfs.object.put("json", Arrays.asList(object.toJSONString().getBytes()));
        //MerkleNode links = udfs.object.links(pointer);
        //byte[] data = udfs.object.data(pointer);
        //Map stat = udfs.object.stat(pointer);
